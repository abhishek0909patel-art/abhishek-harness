...
_SAFE_TOP_PACKAGES = {m.split(".")[0] for m in _SAFE_MODULES}
_BANNED_CALLS = {
 ...
 for sub in ast.walk(node):
  if isinstance(sub, ast.Import):
   for alias in sub.names:
    if alias.name.split(".")[0] not in _SAFE_TOP_PACKAGES:
     raise ValueError(f"import of {alias.name} is not allowed")
  elif isinstance(sub, ast.ImportFrom):
   if (sub.module or "").split(".")[0] not in _SAFE_TOP_PACKAGES:
...
def run_sandbox(
 ...
 namespace: dict[str, Any] = {
  ...
  "pd": pd,
  "np": __import__("numpy"),
  "pl": __import__("plotly.express", fromlist=["express"]),
 ...
 chart = fig.to_dict() if fig is not None else None
